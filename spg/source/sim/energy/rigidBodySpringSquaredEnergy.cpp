...
auto l_springRBConstraint =
    [](const RBSpringSquaredEnergy *energy, const int i, const RigidBodyGroup &rbGroup, auto &dC) {
        ...
        const Real L0Squared = energy->squaredRestLengths()[i];
        dC[0] = (globalRBPoint0 - globalRBPoint1).squaredNorm() - L0Squared;
    };
}  // namespace

void RBSpringSquaredEnergy::addStencil(std::array<int, s_stencilSize> stencil,
                                       ...
                                       const Real stiffness = 0)
{
    ...
    if (restLength < 0) {
        throw std::runtime_error("Negative rest length invalid");
    }
    m_squaredRestLength.push_back(restLength * restLength);
...
void RBSpringSquaredEnergy::dConstraints(const int i, const RigidBodyGroup &rbGroup, ConstraintsAD1 &dC) const
{
    l_springRBConstraint(this, i, rbGroup, dC);
}

void RBSpringSquaredEnergy::dConstraints(const int i, const RigidBodyGroup &rbGroup, ConstraintsAD2 &dC) const
{
    l_springRBConstraint(this, i, rbGroup, dC);
}
}  // namespace spg